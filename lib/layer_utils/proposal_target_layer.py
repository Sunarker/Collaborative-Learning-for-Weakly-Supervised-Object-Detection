...
def proposal_target_layer (rpn_rois, rpn_scores, gt_boxes, _num_classes, gt_weights):
  """
  ...
  # targets
  labels, rois, roi_scores, bbox_targets, bbox_inside_weights, loss_weights = _sample_rois(
    all_rois, all_scores, gt_boxes, gt_weights, fg_rois_per_image,
  ...
  bbox_outside_weights = bbox_outside_weights * loss_weights.view(-1,1)
  #print(bbox_outside_weights)

  return rois, roi_scores, labels, Variable(bbox_targets), Variable(bbox_inside_weights), Variable(bbox_outside_weights), Variable(loss_weights)
